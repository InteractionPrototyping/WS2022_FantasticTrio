  ...
  <f7-page name="project">
    ...
    <f7-block>
      ...
      <div style="position: absolute; bottom: 200px; right: 20px">
        <f7-icon
          v-bind:f7="getItem.like_type"
          size="25px"
          class="black_heart"
          @click="ChangeLike()"
        ></f7-icon>
        <span style="position: relative; top: 5px; font-size: 20px">
          {{ getItem.likes }}
        </span>
      </div>
      <a class="link" href="/profile-helper/">
        <f7-list
          inset
          media-list
          style="height: 100px; position: relative; width: 325px"
        >
          <f7-list-item
            v-bind:title="getItem.writer"
            text="Expert of car repair"
          >
            <br />
            <template #media>
              <img
    ...
    <f7-block>
      ...
      >
        ...
        >
          <template #media>
            <img v-bind:src="latestComment.avatar" width="28" class="avatar" />
          </template>
        </f7-list-item>
      </f7-list>
      <f7-button fill href="/comment/" style="position: relative; bottom: 40px"
...
export default {
  ...
  mounted() {
    // Get project content from page explore
    myBus.on("Id", (data) => {
      this.getItem = data;
    });
    // Get latest comment from page comment
  ...
  methods: {
    ...
    // Change the status of liked dynamically
    ChangeLike() {
      ...
      // Transport change of status to page gallery and explore
